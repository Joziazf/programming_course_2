...
void solutionUniq(double** matrix, int lines, int unk_p, ostream& out) {
    ...
    for (int i = 0; i < lines; i++) {
        ...
        for (int j = 0; j <= unk_p; j++) copy[i][j] = matrix[i][j];
    }
...
int main() {
    ...
    if (fin.fail() || lines <= 0 || unk_p <= 0) {
        cout << "Error incorrect M and N" << endl;
        return 1;
    }
